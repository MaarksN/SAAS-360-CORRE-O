...
# ExpansionMapper - Acceptance Criteria

| Critério | Input mínimo | Output esperado | Como verificar | Evidência |
|---|---|---|---|---|
| Sucesso Nominal | Request com `targetSignalConfidencePct` e segmentos alvo definidos. | Status `success` e schema valido com headline e confianca de sinais de mercado. | Validar contra `ExpansionMapperOutputSchema`. | Testes de unit e schema aprovados. |
