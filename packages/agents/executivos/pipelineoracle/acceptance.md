...
# PipelineOracle - Acceptance Criteria

| Critério | Input mínimo | Output esperado | Como verificar | Evidência |
|---|---|---|---|---|
| Sucesso Nominal | Request com quota target válida e janela temporal preenchida. | Status `success` e schema válido com métricas de rebalanceamento projetadas. | Validar contra `PipelineOracleOutputSchema`. | Testes de unit e schema aprovados. |
