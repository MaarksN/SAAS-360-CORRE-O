...
import {
  ...
  type CapitalAllocatorContract,
  CapitalAllocatorContractSchema,
...
import {
  ...
  createDefaultCapitalAllocatorToolAdapters,
...
interface CapitalAllocatorAgentOptions {
  contractPath?: string;
  now?: () => Date;
  sleep?: (delayMs: number) => Promise<void>;
  toolAdapters?: CapitalAllocatorToolAdapters;
}

interface LoadedContract {
  contract: CapitalAllocatorContract;
...
function classifyContractSource(contractPath: string): LoadedContract["source"] {
  ...
  if (
    normalized.endsWith(
      path
        .join("audit", "pending_review", "ciclo1_capitalallocator", "contract.yaml")
        .toLowerCase()
    )
  ) {
    return "audit_file";
  ...
  if (
    normalized.endsWith(path.join("executives", "CapitalAllocator", "contract.yaml").toLowerCase())
  ) {
    return "file";
  }
  return "custom_file";
...
function parseContractOverridesFromObject(
...
): Partial<CapitalAllocatorContract> {
  const overrides: Partial<CapitalAllocatorContract> = {};
  ...
  if (retry) {
    const retryOverride: Partial<CapitalAllocatorContract["retry"]> = {};
    ...
    if (Object.keys(retryOverride).length > 0) {
      overrides.retry = retryOverride as CapitalAllocatorContract["retry"];
    }
  }
...
function parseContractOverrides(rawText: string): Partial<CapitalAllocatorContract> {
...
function mergeContract(
  overrides: Partial<CapitalAllocatorContract>,
  fallback: CapitalAllocatorContract
): CapitalAllocatorContract {
  return CapitalAllocatorContractSchema.parse({
    ...fallback,
    ...overrides,
    observability: {
...
export class CapitalAllocatorAgent {
  ...
  private readonly toolAdapters: CapitalAllocatorToolAdapters;
  ...
  constructor(options: CapitalAllocatorAgentOptions = {}) {
    this.contractPaths = options.contractPath
      ? [options.contractPath]
      : [...DEFAULT_CONTRACT_PATHS];
    this.now = options.now ?? (() => new Date());
    this.sleepFn = options.sleep ?? sleep;
    this.toolAdapters = options.toolAdapters ?? createDefaultCapitalAllocatorToolAdapters();
  }
  ...
  async run(input: CapitalAllocatorInput): Promise<CapitalAllocatorOutput> {
    const parsedInput = CapitalAllocatorInputSchema.parse(input);
    ...
    emitEvent({
      details: {
        source: "request",
        toolId: "capitalallocator"
      },
      level: "info",
      message: "CapitalAllocator request accepted.",
      name: "capitalallocator.request.received"
    });
    ...
    emitEvent({
      ...
      name: "capitalallocator.contract.loaded"
    });
    ...
    const runWithRetry = async <T>(
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        emitEvent({
          ...
          name: "capitalallocator.tool.call.started"
        });
        ...
        try {
          ...
          emitEvent({
            ...
            name: "capitalallocator.tool.call.succeeded"
          });
          return result;
        } catch (error) {
          const asError =
          ...
          emitEvent({
            ...
            name: "capitalallocator.tool.call.failed"
          });
          ...
          emitEvent({
            ...
            name: "capitalallocator.retry.scheduled"
          });
    ...
    if (fallbackApplied) {
      emitEvent({
        ...
        level: "warning",
        message: "Fallback behavior applied for CapitalAllocator output.",
        name: "capitalallocator.fallback.applied"
      });
    }
    ...
    const status: CapitalAllocatorOutput["status"] =
    ...
    emitEvent({
      ...
      message: "CapitalAllocator response generated.",
      name: "capitalallocator.response.generated"
    });

    const output = CapitalAllocatorOutputSchema.parse({
      agent: "CapitalAllocator",
      budgetBrief: {
        actions: [
          {
            owner: "CFO",
      ...
      summary: fallbackApplied
        ? "CapitalAllocator generated under fallback mode due to tool failures."
        : "CapitalAllocator generated with complete budget telemetry and scenario coverage."
    });
