...
export const BrandGuardianInputSchema = z
  .object({
    constraints: z
      .object({
    ...
    targetBrandSentimentPct: z.number().min(1).max(100),
  ...
  .strict();
export type BrandGuardianInput = z.infer<typeof BrandGuardianInputSchema>;
...
export const BrandEventNameSchema = z.enum([
  "brandguardian.request.received",
  "brandguardian.contract.loaded",
  "brandguardian.tool.call.started",
  "brandguardian.tool.call.succeeded",
  "brandguardian.tool.call.failed",
  "brandguardian.retry.scheduled",
  "brandguardian.fallback.applied",
  "brandguardian.response.generated"
]);
...
export const BrandGuardianContractSchema = z
  .object({
  ...
  .strict();
export type BrandGuardianContract = z.infer<typeof BrandGuardianContractSchema>;

export const DEFAULT_BRANDGUARDIAN_CONTRACT: BrandGuardianContract = {
...
export const BrandGuardianOutputSchema = z
  .object({
    agent: z.literal("BrandGuardian"),
    brandBrief: z
      .object({
        actions: z.array(BrandActionSchema),
        headline: z.string().min(1),
        projectedBrandHealthScore: z.number(),
