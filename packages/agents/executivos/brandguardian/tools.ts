...
export interface BrandGuardianToolAdapters {
...
export function createDefaultBrandGuardianToolAdapters(): BrandGuardianToolAdapters {
  return {
    ...
    async fetchPRIncidents(input: BrandToolInput): Promise<PRIncidentSnapshot> {
      BrandToolInputSchema.parse(input);
      const seed = `${input.tenantId}:${input.targetBrandSentimentPct}:pr`;
