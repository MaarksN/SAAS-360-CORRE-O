...
export interface TrendCatcherToolAdapters {
...
export function normalizeTrendToolId(toolId: string): TrendToolId | null {
  ...
  if (normalized === "market-trend-feed") {
    return "market-trend-feed";
  }
  if (normalized === "social-signal-stream") {
    return "social-signal-stream";
  }
  if (normalized === "category-growth-engine") {
    return "category-growth-engine";
  }
  return null;
}

export function createDefaultTrendCatcherToolAdapters(): TrendCatcherToolAdapters {
  return {
    async fetchCategoryGrowth(input: TrendToolInput): Promise<CategoryGrowthSnapshot> {
      TrendToolInputSchema.parse(input);
      const seed = `${input.tenantId}:${input.targetGrowthPct}:category`;
