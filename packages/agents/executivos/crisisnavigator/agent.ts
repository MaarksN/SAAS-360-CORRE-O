...
export class CrisisNavigatorAgent {
  ...
  constructor(options: CrisisNavigatorAgentOptions = {}) {
    ...
    this.toolAdapters = options.toolAdapters ?? createDefaultCrisisNavigatorToolAdapters();
  }
  ...
  async run(input: CrisisNavigatorInput): Promise<CrisisNavigatorOutput> {
    ...
    const toolInput: CompetitorToolInput = {
      ...
      targetRecoveryPct: parsedInput.targetRecoveryPct,
    ...
    if (effectiveTools.includes("incident-intel-feed")) {
      try {
        competitorIntel = CompetitorIntelSnapshotSchema.parse(
          await runWithRetry("incident-intel-feed", () =>
            this.toolAdapters.fetchCompetitorIntel(toolInput)
          )
        );
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "incident-intel-feed failed.";
        fallbackReasons.push(`incident-intel-feed: ${message}`);
      }
    }

    if (effectiveTools.includes("impact-benchmark-engine")) {
      try {
        pricingBenchmark = PricingBenchmarkSnapshotSchema.parse(
          await runWithRetry("impact-benchmark-engine", () =>
            this.toolAdapters.fetchPricingBenchmark(toolInput)
          )
        );
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "impact-benchmark-engine failed.";
        fallbackReasons.push(`impact-benchmark-engine: ${message}`);
      }
    }

    if (effectiveTools.includes("resilience-gap-analyzer")) {
      try {
        featureGap = FeatureGapSnapshotSchema.parse(
          await runWithRetry("resilience-gap-analyzer", () =>
            this.toolAdapters.fetchFeatureGap(toolInput)
          )
        );
      } catch (error) {
        const message =
          error instanceof Error ? error.message : "resilience-gap-analyzer failed.";
        fallbackReasons.push(`resilience-gap-analyzer: ${message}`);
      }
    }
    ...
    const projectedRecoveryPct = Number(
      (
    ...
    const recommendedResponseFront =
